...
@Component({
  ...
  template: `
    ...
    <nz-spin [nzSpinning]="loading()">
      ...
      <nz-card nzTitle="Solution Architect role" class="sa-card">
        <p class="muted small">
          Full control over board design, agent approval, prompts, and MCP
          servers. Read-only view shown for clarity — the SA role is not
          configurable.
        </p>
        <div class="sa-grid">
          @for (k of saScopes; track k) {
            <code class="mono sa-pill">{{ k }}</code>
          }
        </div>
      </nz-card>
    </nz-spin>
  ...
  styles: [`
    ...
    .sa-grid { display: grid; grid-template-columns: repeat(2, max-content); gap: 8px 12px; }
    .sa-pill {
      background: var(--c-surface-2);
      border: 1px solid var(--c-border-subtle);
      padding: 3px 8px; border-radius: 4px;
      font-size: 12px; color: var(--c-primary); width: max-content;
    }
  `],
})
export class PermissionsComponent implements OnInit {
  ...
  // Static, read-only — mirrors the SA capabilities the backend grants the
  // Solution Architect role. Shown for clarity (the role isn't editable).
  readonly saScopes = [
    'org.*', 'project.*', 'team.*', 'agent.approve',
    'agent.stop', 'prompt.write', 'mcp.write', 'role.write',
  ];

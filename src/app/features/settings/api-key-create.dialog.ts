...
@Component({
  ...
  template: `
    ...
    <form class="form" nz-form nzLayout="vertical" (submit)="submit($event)">
      ...
      <nz-form-item>
        <nz-form-label>Expires</nz-form-label>
        <nz-form-control>
          <nz-select [(ngModel)]="expiresDays" name="expiresDays">
            <nz-option [nzValue]="90" nzLabel="90 days (recommended)"></nz-option>
            <nz-option [nzValue]="30" nzLabel="30 days"></nz-option>
            <nz-option [nzValue]="180" nzLabel="180 days"></nz-option>
            <nz-option [nzValue]="365" nzLabel="1 year"></nz-option>
            <nz-option [nzValue]="0" nzLabel="Never expires"></nz-option>
          </nz-select>
        </nz-form-control>
      </nz-form-item>
    </form>
...
export class ApiKeyCreateComponent implements OnInit {
  ...
  name = '';
  orgId = '';
  /** 0 = never; otherwise days from now. Drives the Expires select. */
  expiresDays = 90;
  ...
  async submit(ev?: Event) {
    ...
    try {
      const body: { name: string; orgId: string; expiresAt?: string } = {
        name: this.name,
        orgId: this.orgId,
      };
      if (this.expiresDays > 0) {
        const d = new Date();
        d.setDate(d.getDate() + this.expiresDays);

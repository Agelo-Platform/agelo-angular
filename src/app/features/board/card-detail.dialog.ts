...
export class CardDetailComponent implements OnInit {
  ...
  async saveFields() {
    ...
    const patches = c.type.customFields
      ...
      .map((f) => {
        ...
        // the backend stores them as CSV; serialize on the way out.
        if (f.type === 'tags' || f.type === 'multi_select') {
  ...
  /** Date-picker binds to a Date; convert from the stored ISO string. */
  dueDateModel(): Date | null {
    const d = this.card()?.dueDate;
    return d ? new Date(d) : null;
  }

  async setPriority(priority: string) {
    const c = this.card();
    if (!c || (c.priority || 'none') === priority) return;
    this.busy.set(true);
    try {
      await firstValueFrom(this.api.patch(`/cards/${c.id}`, { priority }));
      await this.refresh();
      this.toast.success('Priority updated');
    } catch (err: any) {
      this.toast.error(err?.error?.message || 'Could not update priority');
    } finally {
      this.busy.set(false);
    }
  }

  async setDueDate(date: Date | null) {
    const c = this.card();
    if (!c) return;
    const iso = date ? date.toISOString() : null;
    if ((c.dueDate ?? null) === iso) return;
    this.busy.set(true);
    try {
      await firstValueFrom(this.api.patch(`/cards/${c.id}`, { dueDate: iso }));
      await this.refresh();
      this.toast.success('Due date updated');
    } catch (err: any) {
      this.toast.error(err?.error?.message || 'Could not update due date');
    } finally {
      this.busy.set(false);
    }
  }
   ...
   * picker. Accepts the CSV string, an already-parsed array, or null.
   */
